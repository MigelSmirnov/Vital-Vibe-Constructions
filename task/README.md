...
1. Проверить `git status -sb` и актуальность ветки `agent/solar-collector-port`. Сохранить чужие незавершённые изменения; не применять и не удалять stash вслепую.
