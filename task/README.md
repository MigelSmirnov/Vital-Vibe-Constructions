...
1. Проверить `git status -sb` и актуальность ветки `release/vps-migration`. Сохранить чужие незавершённые изменения; не применять и не удалять stash вслепую.
2. Прочитать [AGENTS.md](../AGENTS.md) и перечисленные там контракты в указанном порядке.
3. Первый visual baseline утверждён; accessibility/token layer и текущий bounded responsive-image scope зелёные. История image work — в [002 — responsive image optimization](002-responsive-images.md).
...
## Редакционное правило владельца

Статьи сохраняют исходный авторский текст и логику. Не превращать их в сокращённые отчёты по фото. Восстановлены полный исходник о покраске и полная статья о стоимости ремонта с разделами про полы; фотографии иллюстрируют текст.
...
- Accessibility/token QA зелёный; последний reduced-motion follow-up — run #68 / `c583479bfbd639856143212c52876e949b5ccf40`.
...
- Homepage hero/LCP: baseline run #63 показал один JPEG 264,764 B и hero как LCP в 3/3 viewport. После 480/768/1152 WebP run #64 выбирает 28,102 B / 28,102 B / 55,556 B; synthetic comparison LCP снизился с 1,892/3,044/3,240 ms до 632/640/892 ms. Это не field CWV.
- Галерея сохраняет все 39 contract media. Для 16 самых тяжёлых источников повторно используются уже существующие Standard/Premium derivatives: исходный bounded payload 39,956,827 B, выбранный Chromium payload в run #66 — 257,852 B на каждом reference viewport. Остальные 23 изображения намеренно не подвергались массовой генерации вариантов.
- Shared secondary reduced-motion хвост закрыт: release CSS теперь вычисляет `scroll-behavior: auto` при `prefers-reduced-motion: reduce`; dedicated check проверяет `/gallery/` и El Raval и входит в полный `tools/visual/run.sh`. Run #68 зелёный.
...
| --- | --- | --- |
| Следующий шаг | [Crawler / AEO](../architecture/crawler-aeo-audit/report.md) | Аудит готов: добавить /en/ и /ru/ в sitemap, определить noindex для preview, проверить индексацию через кабинеты поисковиков. Production не изменён. |
...
| Опционально | Оставшиеся 23 gallery images | Не трогать без нового измеренного основания; массовый 39×N проход сознательно не делался. |
| Готово | Каталог статей ES / EN / RU | Автоматические карточки опубликованных статей, пункт меню и обратные ссылки; проверки зелёные. |
| Выполнено | Солнечный коллектор: статья + анимация v3 | Исправлено исчезновение iframe на широких экранах; 18 browser cases ES/EN/RU до 2560 px прошли локально и в production. Релиз 13f0f47. |
| Готово | [Доставка фото статей](../architecture/article-photo-delivery/report.md) | 8 фото, 24 WebP; 36 browser cases локально и 36 в production. Релиз 2695336; текст и внешний вид сохранены. |
| На проверке | Статьи о покраске и полах | ES/EN/RU, 8 фото владельца, цена покраски только за работу; 3 объекта в статье о полах разделены. Полный check зелёный. |
| Отложено | Подготовить выбранный логотип к внедрению | Концепт выбран, **пока не устанавливать на сайт**. |
| Выполнено | [004 — разбор веток и релиз](004-release-branch-audit.md) | История main включена в релизную ветку без изменения файлов сайта; исходная ветка сохранена. |
| Выполнено | [003 — VPS cutover](003-vps-cutover.md) | fb31a282 опубликован на основном домене; HTTPS, автоматическое продление, маршруты и анимация проверены. GitHub Pages сохранён для отката; отдельные security/monitoring follow-ups описаны в architecture/vps-cutover-20260913/report.md. |
...
- `bash tools/visual/run.sh` — browser QA + Standard/Premium project checks + gallery responsive check + accessibility QA + dedicated reduced-motion check + responsive-resource verification + hero/LCP measurement.
